...
parser.add_argument(
    ...
    choices=['wrn', 'allconv', 'densenet', 'resnext', 'resnet18', 'resnet18_pre', 'convnext_tiny', 'convnext_tiny_pre'],
...
parser.add_argument(
    ...
    '-adamw',
    action='store_true',
    help='Uses AdamW with cosineannealing scheduler else SGD with lambda lr_scheduler')
# own code
...
def main():
  ...
  if args.model == 'resnet18_pre':
    ...
    preprocess = ConvNeXt_Tiny_Weights.DEFAULT.transforms()
  else:
    preprocess = transforms.Compose(
      [transforms.ToTensor(),
       transforms.Normalize([0.5] * 3, [0.5] * 3)])
  ...
  # own code
    ...
    #net.classifier = torch.nn.Sequential(norm_layer(768), torch.nn.Flatten(1), torch.nn.Linear(768, num_classes))
    net.classifier[2] = torch.nn.Linear(768, num_classes)
  elif args.model == 'convnext_tiny_pre':
  ...
  if args.use_adamw:
    ...
    optim_used = "SGD"
  writer = SummaryWriter(log_dir='runs/' + args.model + '_' + args.dataset + '_' + optim_used + '_' + 'lr_' + str(args.learning_rate) + '_' + 'epochs_' + str(args.epochs))
  # own code
